  ...
  describe 'usasearch:twitter' do
    ...
    describe 'usasearch:twitter:stream' do
      ...
      context 'when connecting to Twitter' do
        ...
        let(:active_twitter_ids) { [123] }
        let(:active_twitter_ids_string) { active_twitter_ids.map(&:to_s).join(',') }
        ...
        before(:each) do
          ...
          allow(Twitter::Streaming::Client).to receive(:new).and_return @client
        ...
        it 'get a list of all the TwitterProfile ids, setup various callbacks, and call follow' do
          expect(@client).to receive(:filter).with(follow: active_twitter_ids_string)
        ...
        it 'should create a new tweet for every status received' do
          ...
          parsed_json = JSON.parse(tweet_status_json)
          clean_json = parsed_json.symbolize_keys

          # DEBUG
          puts "tweet_status_json: #{tweet_status_json.inspect}"
          puts "parsed_json: #{parsed_json}"
          puts "clean_json: #{clean_json}"

          tweet = Twitter::Tweet.new(clean_json)

          expect(@client).to receive(:filter).with(follow: active_twitter_ids_string).and_yield(tweet)
